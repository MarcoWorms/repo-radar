...
class PRMonitor:
    ...
    def send_summary(self, c, cid, summary, pr_url):
        try:
            c.bot.send_message(cid, f"*PR Summary:* {summary}\n\n*PR Link:* {pr_url}", parse_mode='Markdown')
        ...
        for o_name in gh_orgs:
            ...
            for repo in self.org_cache[o_name]:
                ...
                for pr in pr_list:
                    ...
                    f_summary = self.process_pr(pr, cid)

                    print(f"Sending summary: {f_summary}")
                    self.send_summary(c, cid, f_summary, pr.html_url)
